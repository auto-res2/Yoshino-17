...
Removed unused `# type: ignore` markers and added the missing `datasets` import
for accurate type annotations.
...
################################################################################
# Resilient LLM backend (vllm preferred, dummy fallback) ########################
################################################################################

try:
    from vllm import LLM, SamplingParams

    def _create_llm(model_cfg):
        return (
            LLM(
                model_cfg["name_or_path"],
                tensor_parallel_size=model_cfg.get("tensor_parallel_size", 1),
            ),
            SamplingParams(temperature=model_cfg.get("temperature", 0.7), top_p=0.95),
        )

except Exception as _e:  # noqa: D401 – create *dummy* LLM when vllm missing
    ...
    class _DummyLLM:  # pylint: disable=too-few-public-methods
        def generate(self, prompts, *_, **__):  # noqa: D401
            return [_DummyResp("<dummy-response>") for _ in prompts]

    def _create_llm(model_cfg):  # noqa: D401
        print(
            "[WARN] vllm not available – falling back to dummy responses. "
...
def run_experiment(smoke: bool):  # noqa: D401
    ...
    for guard_name in cfg.guardrails:
        ...
        # LLM instantiation ------------------------------------------------
        model_cfg = cfg.models[sorted(cfg.models.keys())[0]]  # first model entry
        llm, sampler = _create_llm(model_cfg)
        ...
        for dname, ds in datasets_loaded.items():
            ...
            with timeit(f"{dname} generation"):
                for ex in ds:
                    ...
                    outputs.append(resp)
            per_dataset_results[dname] = evaluate_dataset(ds, guard_name, outputs, benign_mask)

        # ----------------------------- aggregation / persistence --------------
        avg_multi = float(np.mean([v["multi_turn_asr"] for v in per_dataset_results.values()]))
        aggregate_metrics[guard_name]["multi_turn_asr"] = avg_multi

        res_path = Path(cfg.output_dir) / f"{guard_name}_results.json"
        save_json(per_dataset_results, res_path)
        print(json.dumps(per_dataset_results, indent=2))

    # ----------------------------------------------- visualise & goodbye ------
    fig_path = Path(cfg.output_dir) / "multi_turn_asr.pdf"
